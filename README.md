...
#### 6.1、实战应用业务场景 - Disruptor与Netty实现百万级场景链接接入
##### 与Netty网络通信框架整合提升性能
...
##### 业务ID生成方式
###### 最使用带有业务含义的ID生成策略，这种方式也在传统应用系统、特定的场景下非常的好用。比如我们现在有一张商品货架表，这张表的数据维度是这样的，比如是按照城市和区域来划分的
...
##### 高并发下的统一ID生成服务
###### 1、如何解决ID生成在并发下的重复生成问题
###### 2、如何承载高并发ID生成的性能瓶颈问题

##### 使用Zookeeper的分布式锁实现统一ID生成策略
###### 不行！ Zookeeper读没什么问题，但是写的时候，只要QPS上千，在并发下肯定是扛不住的，互联网大公司在做统一ID生成服务的时候不会去考虑任何Zookeeper相关的，Zookeeper集群越大，数据同步的能力必须半数以上通过了这个ID才算OK，这个性能瓶颈就被放大了
##### 使用Redis缓存，利用Redis分布式锁实现统一ID生成策略
###### 也不是很好！ Redis本身也会有性能瓶颈，在超高并发下可能会有一些延迟等等细节的问题，无论是主从、哨兵等等都是会有延迟的。Redis服务调用量过长，有时候服务可能会失败，因为期间过程是一个统一调度的生成服务，然后再去调Redis里边的setID等，这个时候就可能出现重复情况，重复情况就可能会出现重试，重试就会影响整个QPS， 就会出现超时，就会调用失败，就会发生订单下了以后订单生成不了

#### 6.3、业界主流的分布式ID生成器的策略是：
##### 实现一、提前加载，也就是预加载的机制 (提前将一批ID生成好，放到内存中，不是缓存中)
###### 1、提前加载，也就是预加载的机制 ，这个思想类似于Disruptor中的预加载机制
###### 2、并发的获取，采用Disruptor框架去提升性能
###### 我们可以预先的生成一批的ID，直接放到应用服务的内存里， 由服务并发地去调用，直接使用就可以了
###### 通过监控ID的使用情况，在ID在快使用完的时候通过定时任务去生成一些ID，
###### 如果想在提前预加载中加入想要的业务规则，比如统一ID生成服务是一个大的容器，可能包含多个业务线：打车业务线、外卖业务线、支付业务线、物流业务线等等，这些都需要有自己的ID规则。用Zookeeper管理配置规则，根据不同的业务按照不同的业务规则生成对应的ID
